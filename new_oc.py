...
class Policy:
    def __init__(self, model, fixed_options=None):
        self.model = model
        ...
        self.fixed_options = None if fixed_options is None else torch.tensor(fixed_options).to(device)
    ...
    def act(self, inputs, options, restart, epsilon=0.01):
...
class EnvOptionPool:
    ...
    def get_next(self, n_steps=100, epsilon=0.1):
        ...
        for i in range(n_steps):
            act = self.policy.act(self.prev_observations, self.prev_options, self.restart, epsilon)
            ...
            cur_rwds_t = torch.tensor(cur_rwds).to(device)[:,None]
...
if __name__ == '__main__':
    ...
    env = make_env('CarIntersect-v1', 'oc_8')
    func_env = partial(make_env, 'CarIntersect-v1', 'oc_8')
    # env = make_env('LunarLander-v2', 'oc_10')
    # func_env = partial(make_env, 'LunarLander-v2', 'oc_10')
    # env = make_env('CartPole-v0', 'oc_4')
    # func_env = partial(make_env, 'CartPole-v0', 'oc_4')
    # env = make_env('Asterix-ram-v0', 'oc')
    # func_env = partial(make_env, 'Asterix-ram-v0', 'oc')
    # env = make_pixel_env('Asterix-v0', 'oc')
    # func_env = partial(make_pixel_env, 'Asterix-v0', 'oc')

    model = Model(env.observation_space.shape[0], env.action_space.n, option_dim=4, hidden_size=256)
    # model = ConvModel(env.observation_space.shape, env.action_space.n, option_dim=8, hidden_size=512).to(device)
